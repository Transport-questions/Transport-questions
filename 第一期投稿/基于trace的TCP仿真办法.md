...
在Letters一文的表述中，轨迹被写成T = {t1, t2, …}，ti为第i个分组的时延，丢失则记为−1；eBPF map以`packet index`为键。测量间隔为10ms，验证时同样使用10ms间隔的irtt，于是一个探测分组一个10ms时隙与轨迹中的一格近似一一对应。回放结果与原始轨迹吻合较好。这一结果说明：在与测量相同的发送节奏下，按包序号回放是自洽的。当时我们将这一对应关系视为默认前提，并在该环境上评测PEPesc。
